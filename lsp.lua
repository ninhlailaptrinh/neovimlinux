...
-- Tạo một bảng để lưu trữ instance của clangd
local server = nil
...
vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function(args)
        if not server then
            ...
            -- Thêm cấu hình dựa trên loại file
            local config = vim.tbl_deep_extend("force", clangd_config, {
                cmd = vim.list_extend(clangd_config.cmd, {
                    "--query-driver=" .. (ft:match("^c$") and "C:/msys64/mingw64/bin/gcc.exe" or "C:/msys64/mingw64/bin/g++.exe")
                }),
                init_options = {
                    fallbackFlags = {
                        ft:match("^c$") and "-std=c11" or "-std=c++17",
                        "--target=x86_64-w64-mingw32",
                        "-I", "C:/msys64/mingw64/include",
                        "-I", "C:/msys64/mingw64/x86_64-w64-mingw32/include",
                        "-I", "C:/msys64/mingw64/lib/gcc/x86_64-w64-mingw32/13.2.0/include",
                        "-I", "C:/msys64/mingw64/lib/gcc/x86_64-w64-mingw32/13.2.0/include-fixed",
                        ft:match("^c$") and "" or "-I C:/msys64/mingw64/include/c++/13.2.0",
                        ft:match("^c$") and "" or "-I C:/msys64/mingw64/include/c++/13.2.0/x86_64-w64-mingw32",
                    },
                    compilationDatabaseDirectory = "build"
                }
            })
            server = vim.lsp.start(config)
        end
    end,
})
